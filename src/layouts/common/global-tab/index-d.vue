...
<script lang="ts">
import { defineComponent, reactive, computed, ref, toRefs, provide, watch, onMounted, nextTick } from 'vue';
...
export default defineComponent({
  ...
  setup(props) {
    ...
    function onScroll(e) {
      const scrollTop =
        e.target.scrollTop || document.documentElement.scrollTop || window.pageYOffset || document.body.scrollTop; // 滚动条偏移量
      state.isMultiHeaderFixed = Boolean(!headerSetting.value.fixed && multiTabsSetting.value.fixed && scrollTop >= 64);
    }
